...
class InvoiceManager extends AbstractManager
{
    ...
    public function insert(array $invoice, int $userId): int
    {
        ...
        $invoiceStatement->bindValue(':status', $status, PDO::PARAM_STR);
     ...
     */
    public function updateStatus(int $invoiceId): void
    {
        $statusQuery = "UPDATE " . self::TABLE . " SET status = 'payée' WHERE id = :id";
        $statusStatement = $this->pdo->prepare($statusQuery);
        $statusStatement->bindParam(':id', $invoiceId, PDO::PARAM_INT);
    ...
    {
        $query = "SELECT created_at, due_at, client_name, total_amount, status, id
